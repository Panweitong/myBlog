...
public class TaleUtils {
    ...
    public static void updateJDBCFile(String url, String dbName, String userName, String password) {
        ...
        try {
            fos = new FileOutputStream("jdbc.properties");
...
//            默认是classPath路径
     ...
     * @return
     */
    public static FontUserVo getLoginFontUser(HttpServletRequest request) {
        HttpSession session = request.getSession();
        if (null == session) {
            return null;
        }
        return (FontUserVo) session.getAttribute(WebConst.FONTLOGIN_SESSION_KEY);
    }
...
//            response.sendRedirect(Commons.site_url());
    ...
    public static String getUplodFilePath() {
        String path = PropertiesUtil.getProperty("attachment.realPath");
